    ...
    public static partial class Authentication {
        ...
        public static void Login(string hexID, NetIncomingMessage data) {
            ...
            if (Configuration.DisableLogin) {
                LoginPacket.Message(hexID, (int)PacketList.LS_CL_Maintenance);
                return;
            }
            ...
            //obtem a referência do usuário para adicionar os dados.
            ...
            if (Accounts_DB.IsBanned(Accounts_DB.GetID(pData.Username))) {
                LoginPacket.Message(pData.HexID, (int)PacketList.LS_CL_AccountBanned);
                return;
            }
            ...
            if (!Accounts_DB.ExistAccount(pData.Username)) {
                LoginPacket.Message(pData.HexID, (int)PacketList.LS_CL_InvalidNamePass);
                return;
            }

            //Verifica se o usuário está ativo, caso falso, envia mensagem de erro
            if (!Accounts_DB.IsActive(pData.Username)) { 
                LoginPacket.Message(pData.HexID, (int)PacketList.LS_CL_AccountDisabled);
                return;
            }
            ...
            if (WorldNetwork.IsWorldServerConnected())
                ...
                //Se não há nenhum servidor conectado, continua para o próximo método
        ...
        public static void Login(bool result, string username) {
            ...
            Logs.Write($"User Login: {pData.Account} {pData.IP}", Color.Black); 
            ...
            LoginPacket.GameState(pData.HexID, GameState.Server);
        }
        ...
        public static void TryingToAccess(PlayerData pData) {       
            ...
            if (pData.LoginAttempt >= Constant.MAX_ATTEMPT) {

                //Desconecta o usuario em todos os servidores
                WorldPacket.DisconnectPlayer(pData);
                ...
                // envia msg 
                LoginPacket.Message(pData.HexID, (int)PacketList.LS_CL_AlreadyLoggedIn);
            }
            else {
