    ...
    public class GameNetwork {
        ...
        public static void InitializeGameServer() {
            for (var index = 0; index < Constant.MAX_SERVER; index++) {
                ...
                if (!string.IsNullOrEmpty(Configuration.GameServer[index].Name)) {
                    GameServer[index].InitializeClient(
                        Configuration.GameServer[index].GameServerLocalIP, 
                        Configuration.GameServer[index].GameServerIP,
                        Configuration.GameServer[index].GameServerPort
                        );
                }
            }
        }
