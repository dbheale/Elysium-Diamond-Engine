...
namespace WorldServer.MySQL {
    public class Character_DB {
        ...
        public static void PreLoad(PlayerData pData) {
            var varQuery = "SELECT classe_id, char_slot, name, sprite, level FROM players WHERE account_id=?accountID";
            var cmd = new MySqlCommand(varQuery, Common_DB.Connection);
        ...
        public static void InsertInitialItems(string name, int classeID) {
            ...
            for(var item = 0; item < Constant.MAX_ITEM; item++) {
                //se não há nenhum item, próximo
                if (Classe.Classes[index].GetItem((ItemType)item).ID == 0) continue;

                varQuery.Append("INSERT INTO player_equippeditem (char_id, item_id, unique_id, equip_slot, quantity, ");
                varQuery.Append("enchant, element, durability, slots, expire, expire_time, soul_bound) VALUES (");
                ...
                varQuery.Append($"'{item}', ");
                varQuery.Append($"'{Classe.Classes[index].GetItem((ItemType)item).Quantity}', ");
                ...
                varQuery.Append($"'{Classe.Classes[index].GetItem((ItemType)item).Expire}', ");
                varQuery.Append($"'{Classe.Classes[index].GetItem((ItemType)item).ExpireTime.ToString("yyyy/M/d HH:mm:ss")}', ");
                varQuery.Append($"'{Classe.Classes[index].GetItem((ItemType)item).SoulBound}') ");
        ...
        public static int GetGuildID(int playerID) {
            var varQuery = "SELECT guild_id FROM players WHERE id=?playerID";
            var cmd = new MySqlCommand(varQuery, Common_DB.Connection);
        ...
        public static void UpdatePlayerGuildID(int playerID, int guildID) {
            var varQuery = "UPDATE players SET guild_id=?guildID WHERE id=?playerID";
            var cmd = new MySqlCommand(varQuery, Common_DB.Connection);
            cmd.Parameters.AddWithValue("?playerID", playerID);
